...
Reads the two Hamilton College spreadsheets and outputs:
- per-building yearly energy
- campus-wide yearly totals
- a cleaned 2023 snapshot with square footage
...
def extract_one_year(df):
    # go row by row since the file format is pretty messy
    # returns: { building: {kwh, gas_cuft} }
    ...
    for i in range(len(df)):
        ...
        if col0 and col0 not in ["NAN", ""] and col1 in ["KWH", "MWH", "MW"]:
            ...
            for col_idx in range(2, min(14, len(df.columns))):
                try:
                    ...
                    # missing or weird values → just treat as 0
        ...
        # gas row usually comes right after the building row
...
def load_meter_log(filepath):
    # reads all sheets (each one is a year)
...
def load_sq_footage(filepath):
    # returns: { building: {sqft, fuel, use, year_built} }

    print(f"\nReading square footage file: {filepath}")
    df = pd.read_excel(filepath)

    # clean column names (there was a trailing space issue before)
...
def build_buildings_table(year_data):
    # flatten into a dataframe: one row per building per year
...
def build_campus_totals(buildings_df):
    # sum everything into one row per year
...
def build_2023_snapshot(buildings_df, sq_dict):
    ...
    # convert sq_dict to dataframe for easier mapping
    sq_rows = []
    ...
    # try to match building names between files
    ...
    def best_match(meter_name):
        ...
        for sq_name in sq_dict.keys():
            ...
            # if they share a couple words, assume it's the same building
            if len(meter_words & sq_words) >= 2:
                return sq_name

            # fallback for short names like "gym", "chapel"
    ...
    df_2023["sqft"] = df_2023["sq_name"].map(
        lambda n: sq_dict.get(n, {}).get("sqft")
    )
    df_2023["fuel"] = df_2023["sq_name"].map(
        lambda n: sq_dict.get(n, {}).get("fuel")
    )
    df_2023["use"] = df_2023["sq_name"].map(
        lambda n: sq_dict.get(n, {}).get("use")
    )
    df_2023["year_built"] = df_2023["sq_name"].map(
        lambda n: sq_dict.get(n, {}).get("year_built")
    )

    df_2023 = df_2023.drop(columns=["year", "sq_name"])

    # drop buildings with no energy data
    df_2023 = df_2023[df_2023["kwh"] + df_2023["gas_cuft"] > 0]

    return df_2023.reset_index(drop=True)
